...
$('.img0').click(function() {
	...
	inall.style.backgroundImage = "url(images/info/allbg.png)";
...
function admin() {
	...
	$("#btnLogin0").click(function() {//时间绑定到登录按钮
							...
							a = new $.pos(
									{
										...
										formtitle : '<div class="con_tit dt"><div class="l gkt1">编号：<span>'
												+ $(this).attr('no')
												+ '  '
												+ $(this).attr('state')
	...
	if (r.success) {
		if (r.status == '3') {
			if (r.type == 1010) {
				...
				//document.getElementById("em7").stop();
			} else {
				alert('走支付流程');
			}
		} else {
...
function expgridopen() {
	...
	$('body')
			.append(
					'<embed src="mp3/'
							+ door
							+ '号柜门已经打开，请取走您的包裹.mp3" autostart="true" hidden="true" loop="false">');
	$("#door").html(door);
